...
describe('CaixaDaLanchonete', () => {
   ...
   test.each([
    ...
    test.each([
        ...
        test.each([
            ['combo 1 com chantily', 'dinheiro', 'Item extra não pode ser pedido sem o principal, pois combos não são considerados principais', ['combo1,1', 'chantily,1']],
            ['combo 2 com queijo', 'credito', 'Item extra não pode ser pedido sem o principal, pois combos não são considerados principais', ['combo2,1', 'queijo,2']],
        ])('compra de %p em %p deve resultar em %p', (_, formaDePagamento, resultadoEsperado, itens) =>
            validaTeste(formaDePagamento, resultadoEsperado, itens));


            test.each([
                ['pedido com dois chantilys e um café', 'dinheiro', 'R$ 5,70', ['cafe,1', 'chantily,2']],
                ['pedido com três queijos e um sanduíche', 'credito', 'R$ 12,88', ['sanduiche,1', 'queijo,3']],
            ])('compra %p em %p deve resultar em %p', (_, formaDePagamento, resultadoEsperado, itens) =>
            validaTeste(formaDePagamento, resultadoEsperado, itens));
